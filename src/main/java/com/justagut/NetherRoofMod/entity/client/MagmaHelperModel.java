...
public class MagmaHelperModel<T extends MagmaHelperEntity> extends HierarchicalModel<T> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath(netherroofmod.MODID, "magmahelper"), "main");
    private final ModelPart body;

    public MagmaHelperModel(ModelPart root) {
    ...
    @Override
    public void setupAnim(MagmaHelperEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.root().getAllParts().forEach(ModelPart::resetPose);


        this.animateWalk(MagmaHelperAnimations.WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
